    ...
    var datatable = {
        investigacionasesoriaAsesoria: {
            ...
            options: {
                ...
                lengthChange: false,
                ajax: {
                    url: "/Investigacionasesoria/asesoriadocente".proto().parseURL(),
                    type: "GET",
                    dataType: "JSON",
                    ...
                    },
                    {
                        ...
                        render: function (data) {
                            var template = "";
                                  ...
                                  //conf
                                template += "<a title='Configurar' ";
                                template += " style='background-color:#aaeee8;color:#000000' ";
                                template += " class='btn  btn-conf ";
                            template += "m-btn btn-sm  m-btn--icon-only' ";
                            template += " href='/Investigacionasesoria/asesoriadocentedetalle?id=" + data.id + "'";
                                template += " data-id='" + data.id + "'>";
                                template += "<i class='la la-book'></i> Configuración</a>";
            ...
            swal({
                ...
                preConfirm: () => {
                    return new Promise((resolve) => {                        
                        $.ajax({
                            url: ("/Investigacionasesoria/asesoriadocente?handler=Delete").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
        ...
        enviar: function (id) {
            swal({
                ...
                preConfirm: () => {
                    return new Promise((resolve) => {
                        $.ajax({
                            url: ("/Investigacionasesoria/asesoriadocente?handler=EnviarPlan").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
