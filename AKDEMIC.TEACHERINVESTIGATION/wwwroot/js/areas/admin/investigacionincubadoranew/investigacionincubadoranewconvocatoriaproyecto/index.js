var InitApp = function () {
    ...
    var selectcronogramaproyecto = {
        ...
        researcherCronogramaProyecto: {
            ...
            load: function (idconvocatoriaproyecto= "08dbec43-6a81-40af-88f5-08e40a016e3b") {
                $.ajax({
    ...
    var datatable = {
        investigacionfomentoConvocatoriaproyecto: {
            ...
            options: {
                ...
                lengthChange: false,
                ajax: {
                    url: "/InvestigacionIncubadoranew/convocatoriaproyecto".proto().parseURL(),
                    type: "GET",
                    dataType: "JSON",
                        ...
                        render: function (data) {
                            ...
                            //Subur Carta
                                ...
                                if (data.retornadocente == "1") {
                                    var detailUrl = `/InvestigacionIncubadoranew/convocatoriaproyecto/detalle?id=` + data.id;
                                    template += `<a title='Anexar documentos' class='btn btn-success m-btn btn-sm m-btn--icon-only' href='${detailUrl}' `;
                                    template += `<span><i class='flaticon-file'></i> Subir Anexos</span></span></a> `;
                            ...
                            //Enviar
                            if (data.archivourlcarta != '' && (data.estado == "0" || data.estado == "3x")) {
    ...
    var modal = {
        ...
        addmiembro: {
            ...
            options: {
                ...
                ajax: {
                    ...
                    dataType: "JSON",
                    data: function (data) {
                        data.handler = "DetailPlanMiembros";
                        data.id = $("#editmiembro-form input[name=Id]").val();
                    }
                },
                pageLength: 50,
                orderable: [],
            ...
            load: function (id) {
                ...
                $("#editmiembro-form input[name=Id]").val(id);
                $("#addmiembro-form input[name=IdConvocatoriaproyecto]").val(id);
                if (this.object == null) {
                    this.object = $("#data-tablemiembros").DataTable(this.options);
                } else {
                    this.reload();
                }
                contador2 = 0;
                modal.addmiembro.events();
            ...
            show: function (id) {
                ...
                modal.addmiembro.load(id);
                $("#editModalMiembro").modal("toggle");
            },
            clear: function () {
                // modal.addobservaciones.object.resetForm();
            },
            events: function () {
                ...
                $("#editModalMiembro").on("hidden.bs.modal", function () {
                    modal.addactividades.clear();
                });
                $("#editModalMiembro").on("hidden.bs.modal", function () {
                    modal.addactividades.clear();
                });
                $("#data-tablemiembros").on('click', '.btn-addActividad', function () {
                    var id = $(this).data("id");
                    modal.addactividad.show(id);
                });
                $("#data-tablemiembros").on('click', '.btn-addCronograma', function () {
                    var id = $(this).data("id");
                    var idconvocatoriaproyecto = $(this).data("idconvocatoriaproyecto");
                    $("#editcronograma-form select[name=IdConvocatoriaproyectocronograma]").empty();
                    selectcronogramaproyecto.researcherCronogramaProyecto.load(idconvocatoriaproyecto);
        ...
        editmiembro: {
            object: $("#addmiembro-form").validate({
                submitHandler: function (form, e) {
                    $("#btnMiembro").addLoader();
                    e.preventDefault();
                    let formData = new FormData(form);
                    $.ajax({
                        url: $(form).attr("action"),
                        ...
                        $("#addModalMiembro").modal("hide");
                        $(".m-alert").addClass("m--hide");
                        modal.addmiembro.reload();
                        toastr.success(_app.constants.toastr.message.success.task, _app.constants.toastr.title.success);
                        modal.miembro.clear();
                    }).fail(function (error) {
                        toastr.error(error.responseText, _app.constants.toastr.title.error);
                        $("#btnEditmiembro").removeLoader();
                        //if (error.responseText !== null && error.responseText !== "") $("#add_form_msg_txt").html(error.responseText);
                        //else $("#add_form_msg_txt").html(_app.constants.ajax.message.error);
                        //$("#add_form_msg").removeClass("m--hide").show();
                    }).always(function () {
            ...
            show: function (id) {
                ...
                $("#addmiembro-form input[name=IdConvocatoriaproyecto]").val(id);
                $("#btnMiembro").removeLoader();
                $("#addModalMiembro").modal("toggle");
            },
            clear: function () {
                $("#addmiembro-form").find(".custom-file-label").text("Seleccionar archivo");
                modal.editmiembro.object.resetForm();
            },
            events: function () {
                $("#addModalMiembro").on("hidden.bs.modal", function () {
                    modal.editmiembro.clear();
                });
            }
        },
        addanexo: {
...
//                        modal.newactividadcronograma.clear();
            ...
            swal({
                ...
                preConfirm: () => {
                    return new Promise((resolve) => {                        
                        $.ajax({
                            url: ("/InvestigacionIncubadoranew/convocatoriaproyecto?handler=Delete").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
        ...
        enviar: function (id) {
            swal({
                ...
                preConfirm: () => {
                    return new Promise((resolve) => {
                        $.ajax({
                            url: ("/InvestigacionIncubadoranew/convocatoriaproyecto?handler=EnviarProyecto").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
        ...
        enviaranexo: function (id) {
            swal({
                ...
                preConfirm: () => {
                    return new Promise((resolve) => {
                        $.ajax({
                            url: ("/InvestigacionIncubadoranew/convocatoriaproyecto?handler=EnviarAnexoproyecto").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
        ...
        pdfcartapresentacion: function (id) {
                        ...
                        $.ajax({
                            url: ("/InvestigacionIncubadoranew/convocatoriaproyecto?handler=Pdfcartapresentacion").proto().parseURL(),
                            type: "POST",
                            data: {
                                id: id
                            },
