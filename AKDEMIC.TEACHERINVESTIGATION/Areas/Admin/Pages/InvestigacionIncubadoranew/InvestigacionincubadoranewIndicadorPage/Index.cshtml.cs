...
using AKDEMIC.TEACHERINVESTIGATION.Areas.Admin.ViewModels.InvestigacionFomento.InvestigacionfomentoIndicadorViewModels;
...
namespace AKDEMIC.TEACHERINVESTIGATION.Areas.Admin.Pages.InvestigacionIncubadoranew.InvestigacionIncubadoranewIndicadorPage
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostCreateAsync(InvestigacionfomentoIndicadorCreateViewModel viewModel)
        {
            if (!ModelState.IsValid)
                return new BadRequestObjectResult("Verificar el formulario");

            var maestroarea = new InvestigacionfomentoIndicador
            {
            ...
            await _context.InvestigacionfomentoIndicadores.AddAsync(maestroarea);
            await _context.SaveChangesAsync();

            return new OkResult();
        }

        public async Task<IActionResult> OnPostEditAsync(InvestigacionfomentoIndicadorEditViewModel viewModel)
        {
            if (!ModelState.IsValid)
                return new BadRequestObjectResult("Verificar el formulario");

            var maestroarea = await _context.InvestigacionfomentoIndicadores.Where(x => x.Id == viewModel.Id).FirstOrDefaultAsync();
        ...
        {
            var maestroarea = await _context.InvestigacionfomentoIndicadores.Where(x => x.Id == id).FirstOrDefaultAsync();

            if (maestroarea == null) return new BadRequestObjectResult("Sucedio un error");

            _context.InvestigacionfomentoIndicadores.Remove(maestroarea);
            await _context.SaveChangesAsync();
            return new OkResult();
        }
        ...
        public async Task<IActionResult> OnGetDatatableAsync(string searchValue = null)
        {
            ...
            Expression<Func<InvestigacionfomentoIndicador, dynamic>> orderByPredicate = null;
