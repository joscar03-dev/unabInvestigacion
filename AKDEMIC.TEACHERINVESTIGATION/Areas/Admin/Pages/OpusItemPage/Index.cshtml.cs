...
namespace AKDEMIC.TEACHERINVESTIGATION.Areas.Admin.Pages.OpusItemPage
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public void OnGet()
        {
           ...
           /* var lista = new List<OpusItem>();

            
            var data = _context.OpusItems
                .ToList();
            lista = data;

            foreach(var item in lista )
            {
                Console.WriteLine( item.Name + "ddd" );
        ...
        public async Task<IActionResult> OnGetDatatableAsync(string searchValue = null)
        {
            ...
            var querylist = _context.OpusLists.AsNoTracking();
            var data = await query
                .OrderByCondition(sentParameters.OrderDirection, orderByPredicate)
                .GroupJoin(querylist,
                item1=>item1.IdList,
                list1=>list1.Id,
                (item1, list1) => new {item1, list1})
                .SelectMany(x=>x.list1.DefaultIfEmpty(),
                    (item1, list1) => new { item1, list1 })
                .Select(x => new
                {
                    x.item1.item1.Id,
                    x.item1.item1.Name,
                    x.item1.item1.Sistema,
                    x.item1.item1.Code,
                    nameList =  x.list1.Name,
